...
func New(conf Config, options ...elastic.ClientOptionFunc) (cli *client, err error) {
	...
	if conf.EnableLog {
		options = append(options, elastic.SetErrorLog(logger{goo_log.ERROR}))
		options = append(options, elastic.SetTraceLog(logger{goo_log.DEBUG}))
		options = append(options, elastic.SetInfoLog(logger{goo_log.INFO}))
	}
	...
	if err != nil {
		goo_log.WithTag("goo-es").WithField("host", conf.Addr).WithField("options", options).Error(err)
		return
	}
